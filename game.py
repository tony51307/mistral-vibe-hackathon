...
from economy import EconomyConfig, REASONING_PRICES, affordable_tiers
...
class GameEngine:
    ...
    def play_next_round(self, state: SeasonState) -> RoundResult | None:
        ...
        active_agents = [
            agent for agent in state.agents.values() if not agent.eliminated and agent.bankroll > 0
        ]
        ...
        for agent in active_agents:
            ...
            public_messages[agent.agent_id] = policy.public_message(agent, problem.category)
        ...
        for agent in active_agents:
            ...
            if show_hand:
                ...
                tier, routing_decision = policy.choose_tier(agent, agent.bankroll, context)
