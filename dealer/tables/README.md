...
`table_modes_v1.yaml` is the machine-readable setup for the supported 2-to-10-agent tables. `table_size.md` contains the original 2-to-8 economic rationale; the 9–10-agent extension applies the same linear rule.
...
| `social_lab` | 7 | $21 | $91 | 3, jackpot pressure |
| `tournament` | 8 | $24 | $104 | 3, jackpot pressure |
| `social_stress` | 9 | $27 | $117 | 3, jackpot pressure |
| `ecology` | 10 | $30 | $130 | 3, jackpot pressure |
...
Copy one item in `table_modes_v1.yaml`, give it a unique `id`, and set `initial_agent_count` between 2 and 10. Choose an existing agenda strategy from 1 through 15 and document at least one use case. Strategies 6–15 are showcase-biased dynamic-edge stress tests, not unbiased defaults; strategies 11–15 also require the V2 problem bank and V3 agenda catalog.
